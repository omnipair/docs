...
- **Pessimistic EMA Pricing** Omnipair uses a double-EMA system for collateral valuation: a symmetric EMA (configurable half-life) and a directional EMA that snaps down instantly on price drops but rises gradually. The protocol always uses the more conservative price — $\min(P_{\text{directional}}, P_{\text{symmetric}})$ — ensuring rapid response to adverse price movements while preventing manipulation via temporary price spikes.
- **Impact-Based Collateral Valuation** Rather than using linear price × quantity valuation, collateral is valued by simulating the actual AMM swap needed to liquidate it. This accounts for price impact, ensuring large positions cannot borrow more than their collateral can actually cover in liquidation. Virtual reserves are reconstructed at the pessimistic EMA price while preserving the AMM invariant.
...
- **Dynamic Collateral Factor** Collateral factors (LTV ratios) adjust dynamically based on pool liquidity and existing debt utilization. The CF accounts for price impact of liquidating collateral, automatically tightening for larger positions or when pool utilization is high. A 5% LTV buffer between borrow and liquidation thresholds provides users time to manage positions before liquidation.
...
- Collateral is seized and added to pool reserves, with a **liquidation penalty** benefiting LPs.
- A **liquidator incentive** rewards callers, making liquidation permissionless and reliable.
- The **pessimistic EMA pricing** ensures liquidations trigger before positions become deeply insolvent.
